...
abstract contract Proxy {
    ...
    function _fallback() internal {
        if (
            OpenZeppelinUpgradesAddress.isContract(msg.sender) &&
            msg.data.length == 0 &&
            gasleft() <= 2300
        )
...
interface IProxyFactory {
    ...
    function productImplementations(bytes32 name)
        external
        view
        returns (address);
}
...
contract ProductProxy is Proxy {
    ...
    function productName() public pure virtual returns (bytes32) {
        return 0x0;
    }
    ...
    function _setFactory(address newFactory) internal {
        require(
            OpenZeppelinUpgradesAddress.isContract(newFactory),
            "Cannot set a factory to a non-contract address"
        );
